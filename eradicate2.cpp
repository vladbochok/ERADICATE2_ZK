...
std::string makePreprocessorInitHashExpression(const std::string & strDerivationPrefix, const std::string & strAddressBinary, const std::string & strInitCodeDigest, const std::string & strCalldataDigest) {
	...
	for (int i = 0; i < 32; ++i) {
		h.b[i] = strDerivationPrefix[i];
	}

	for (int i = 0; i < 20; ++i) {
		h.b[i + 44] = strAddressBinary[i];
	}
	...
	for (int i = 0; i < 32; ++i) {
		h.b[i + 96] = strInitCodeDigest[i];
	}

	for (int i = 0; i < 32; ++i) {
		h.b[i + 128] = strCalldataDigest[i];
	}
...
int main(int argc, char * * argv) {
	try {
		...
		std::cout << " KEK " << strPreprocessorInitStructure << std::flush;
